...
# we set the topmost grid of the lower block have the
# same stress of lower most grid of upper block.
stress[0, :, nby0-1] = stress[0, :, nby0]
stress[1, :, nby0-1] = stress[1, :, nby0]
stress[2, :, nby0-1] = stress[2, :, nby0]

# print(stress[0, :, nby0][800], stress[0, :, nby0-1][800])
# print(np.shape(stress[0, :, nby0-1]))
# print(nby0, nby1)

# sys.exit()
# result = fdfault2.output('longterm_initial_mesh','vxbody')
# result.load()
# new_xx = result.x
# new_yy = result.y

# print(np.shape(new_xx), np.shape(new_yy), np.shape(stress[0, :, :]))
# plt.pcolor(new_xx, new_yy, stress[2, :, :], vmax=0.)
# plt.colorbar()
# plt.show()
# sys.exit()
